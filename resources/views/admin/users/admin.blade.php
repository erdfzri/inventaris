...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <div>
            <h2 style="font-size: 1.25rem;">Administrator Accounts</h2>
            <p style="font-size: 0.875rem; color: var(--text-muted);">Manage system administrators</p>
        </div>
        <div style="display: flex; gap: 0.5rem;">
            <a href="{{ route('admin.users.export', 'admin') }}" class="btn btn-secondary">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: right; display: flex; justify-content: flex-end; gap: 0.5rem;">
                        <button onclick="showEditModal({{ json_encode($user) }})" class="btn" style="background: rgba(79, 70, 229, 0.1); color: var(--primary); padding: 0.5rem;"><i class="fas fa-edit"></i></button>
                        @if($user->id !== auth()->id())
                        <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Delete this user?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn" style="background: rgba(239, 68, 68, 0.1); color: var(--danger); padding: 0.5rem;"><i class="fas fa-trash"></i></button>
                        </form>
                        @endif
                    </td>
                </tr>
    ...
    <div class="modal-content">
        ...
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" class="form-control" required placeholder="john@school.id">
            </div>
...
<div id="editModal" class="modal-backdrop">
    <div class="modal-content">
        <h2 style="margin-bottom: 1.5rem;">Edit Admin Account</h2>
